    ...
    with st.sidebar.expander("🧠 Advanced Learning Analytics", expanded=False):
        try:
            ...
            if chatbot.autonomous_trainer:
                ...
                training_stats = {'total_sessions': 0, 'accuracy_rate': 0.0, 'avg_response_time': 0.0}
    ...
    with st.sidebar.expander("⚙️ Training Configuration", expanded=False):
        ...
        with col1:
            ...
            if auto_training != chatbot.auto_training_enabled:
                ...
                if auto_training:
                    st.info("✅ Autonomous training enabled")
                else:
                    st.info("❌ Autonomous training disabled")
            
            # Training interval
            if chatbot.autonomous_trainer and hasattr(chatbot.autonomous_trainer, 'training_interval'):
                interval = st.slider(
                ...
                st.info("⚠️ Autonomous training is not available in this chatbot version")
            
            # Quality threshold
            if chatbot.autonomous_trainer and hasattr(chatbot.autonomous_trainer, 'quality_threshold'):
                threshold = st.slider(
                    "Quality Threshold",
                    min_value=0.5,
                    max_value=1.0,
            ...
            if st.button("🚀 Run Training Session Now"):
                try:
                    if chatbot.autonomous_trainer:
                        ...
                        st.warning("⚠️ Autonomous training is not available in this chatbot version")
                except Exception as e:
...
with col4:
    ...
    if chatbot and hasattr(chatbot, 'get_learning_statistics'):
        try:
            stats = chatbot.get_learning_statistics()
